...
## Очерёдность файлов для изучения

**Со студентами, по порядку:**

1. `README.md` — коротко, для ориентации (что за проект, как подключиться).
2. `docs/KAFKA_TESTING_THEORY.md` — мини-теория в начале звонка (0-5 мин):
   разделы «Быстрая сводка» + 1-5.
3. `docs/DESIGN.md` — по ходу тура по `main` (5-15 мин): архитектура,
   роли сервисов, ручки, топики — как опорная карта, не читать вслух.
4. `docs/TESTER_TASKS.md` — центральный документ хендс-он части (15-70
   мин), по нему студенты тестируют каждую ветку.
5. `docs/LESSON_PLAN.md`, раздел 3 («Теория: CI/CD») — в конце (70-82
   мин), после обеих веток — разбор, почему пайплайн пропустил баги.

**Не для студентов, только тебе:**
- `docs/LESSON_PLAN.md` целиком — твой сценарий/тайминг, не раздавать.
- `docs/HOST_CHEATSHEET.md` (этот файл) — команды пуша/force-push и т.п.
- `HANDOFF.md` — внутренние заметки о разработке, не нужны студентам.
